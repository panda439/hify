<!--
Sync Impact Report
- 版本变化：1.1.0 -> 1.2.0（2026-08-25）
- 修订原则：VIII「Codex / Claude 角色分离」-> VIII「提交时机由所有者决定」。所有者已停用 Codex，
  交接文件已删除，原条款前提不存在；保留"提交时机归所有者"这一内核，并补入"提交前核对实际暂存
  内容、禁止宽泛 git add <目录>"（源于本次两笔提交夹带未跟踪文档的实际教训）。
- 其余原则未变。
...
实现与测试可以自主推进，但**把工作固化进版本历史是所有者的决定**：`git commit` / `git push`
只在所有者明确要求时执行，不擅自提交。

提交前**必须**核对实际被暂存的内容（`git status` 看 `A`/`M` 行本身，而不是只看"有什么没被暂存"），
**禁止**用宽泛的 `git add <目录>` 把未跟踪文件顺带吸进来。提交信息只描述这一笔真实包含的改动，
夹带了别的文件就要在信息里写明，或者拆成另一笔。

汇报实现结果时以当前代码和真实测试输出为准，不把计划文档、口头说明或任何 agent 的报告当作
实现证据——包括自己此前的报告。

> **本条修订记录（v1.2.0，2026-08-25）**：原条款是「Codex / Claude 角色分离」，规定 Codex 主导
> 澄清与审计、Claude 主导实现，协作状态写在 `.claude/CODEX_CLAUDE_HANDOFF.md`。仓库所有者已停止
> 使用 Codex，该交接文件也已删除，整条前提不复存在。保留其中唯一仍然成立的内核（提交时机归所有者），
> 并补入"提交前核对暂存内容"——001-rag-query-rerank 期间有两笔提交因宽泛的 `git add <目录>`
> 夹带了未跟踪文档且提交信息未提及，事后需要重建历史才修正，这条是为防止同类问题复发。
