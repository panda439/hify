...
`internal/eval/retrieval`（纯逻辑：`Hit@1`/`Hit@3`/`MRR`/内容唯一率 + 阈值判定）+ `internal/knowledge/eval_gate_test.go`（`TestRetrievalGatePhase6`，`make eval-retrieval-gate`）是和上面 agent 级 Eval 完全独立的第二套回归工具，专门盯 Phase 3-5/8 的检索质量，不依赖 LLM/Judge：
