...
| 3 Hybrid Search + 邻接分块扩展 + 内容去重 | 单测（`rrfFuse`/`sortVectorCandidatesByScoreThenID` 纯逻辑：双路命中提升排名、去重、单路场景、确定性排序、topK 截断、Score 语义不被 fusionScore 污染、顺序无关、向量候选并列 Score 按 ID 稳定排序，**加 Phase 5：截断到 topK 前先内容去重让内容不同的候选补位、保留融合排名更高的重复项、结果确定性**；`expandWithNeighbors`/`buildNeighborGroups`/`neighborIndexesFor` 纯逻辑：前后邻接块补齐、chunk_index=0/文档末块边界、缺号跳过、核心块排名不变、全部核心块在全部邻接块之前的两层输出布局、邻接块本身是核心块时不重复不提前、共享邻接块归属排名更高的核心块、Score 继承、`NeighborOf` 归属、Citation 元数据不被核心块覆盖、确定性、`anchorCount*3` 上界、空核心/空邻接边界，**加 Phase 5：核心块与邻接块正文重复时保留核心块、邻接块之间重复时保留输出顺序靠前者、去重不改写保留条目的 NeighborOf/Score/Citation 字段**；`normalizeContentForDedup`/`dedupExactContentChunks` 纯逻辑（`dedup_test.go`）：CRLF/首尾空白/行尾空格归一化后判重、大小写/标点/内部缩进不误判、保留最高排名重复项、保留字段不被改写、空输入/单条输入边界、判重键只看内容不看 ID/Score；`conversation/budget.go` 的 `selectEvidence` 预算优先级回归：预算只够两个 source 时邻接块绝不挤掉排名更低的核心块、预算充足时按两层顺序全部保留、预算只够核心块时邻接块全部过滤、预算连核心块都放不下时仍按核心排名依次取舍、邻接块内部顺序变化不影响核心块保留结果）+ 集成（pgvector `<=>` 排序分数/维度过滤/topK/跨模型合并/inactive 跳过/并列 Score 按 ID 稳定；pg_trgm 中英文关键词命中、未发布/跨知识库过滤、维度无关；融合把强关键词命中提进 topK、去重、Citation 元数据完整、空 query/空知识库返回空；`findPublishedNeighborChunks` 前后块查询、跨文档/跨版本隔离、未发布排除、chunk_index=0 无负数查询、`chunk_index ASC, id ASC` 排序、Citation 元数据、`document_version` 在向量/关键词两路都正确返回、文档重新处理后旧版本邻接查询返回空而非串版本、邻接查询普通失败时降级为只返回核心块、context 取消传播；**加 Phase 5（针对真实 Postgres）：内容重复的核心块只保留分数更高的一条并让第三条唯一候选补位、被去重淘汰的核心块绝不触发邻接窗口查询（污染探针验证）、核心块与邻接块正文重复时保留核心块；加 Phase 5（针对 Service.Retrieve 全链路，需要 MySQL，本沙箱按约定 SKIP）：真实 Retrieve 调用下的重复内容去重补位、真实 Retrieve 调用下邻接去重优先保留核心块**） | `knowledge/hybrid_test.go`、`knowledge/neighbor_test.go`、`knowledge/dedup_test.go`、`knowledge/integration_test.go`、`conversation/budget_test.go` |
