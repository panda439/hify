...
| 3 | Hybrid Search + 邻接分块扩展 + 内容去重检索链路（Phase 3 + Phase 4 + Phase 5 + Phase 7） | 由链路 1 / 链路 5 内部触发（`knowledge.Retrieve`） | 向量一路：按 embedding 模型分组 `kbsByModel` → `embedQuery` → `searchVectorChunks`（pgvector `<=>` 余弦距离，打分/排序/candidateK 全下推，返回 `document_version`）；关键词一路：`searchKeywordChunks`（pg_trgm `<%` word-similarity，GIN 索引，不依赖 embedding，同样返回 `document_version`）；两路候选各取 `candidateK = min(max(topK*4, topK), 100)` → `rrfFuse` 按 chunk ID 去重、Reciprocal Rank Fusion 排序 → Phase 5：`dedupExactContentChunks` 在**截断到 topK 之前**对整个融合候选列表做正文内容去重（CRLF/首尾空白/行尾空格归一化后完全相同即判重，大小写/标点/内部缩进不参与判断；保留融合排名更高的一条），去重完成后才截断全局 topK 核心命中（anchors）——这个顺序是"内容不同的候选可以补上被去重掉的名额"的唯一原因 → Phase 4 + Phase 7：`expandWithNeighborWindow` 用 `buildNeighborRequests` 把（已经内容去重过的）anchors 需要的全部邻接坐标（`document_id` + `document_version` + `chunk_index`）展平去重成一个请求集合，**一次** `findPublishedNeighborChunksBatch` 批量查询取回同文档同版本已发布的前一个/后一个 chunk——Phase 7 之前这里是按 `(document_id, document_version)` 分组循环、每组一条 `findPublishedNeighborChunks`（N+1 往返）；被 Phase 5 淘汰的重复核心块不在 anchors 里，因此永远不会出现在请求集合中 → `expandWithNeighbors` 拼装成两个完整分层输出：全部 topK 核心块（保持 RRF 排名）在前，全部邻接块（按所属核心块排名分组，组内 previous/next）整体在后——不是每个核心块紧跟自己的邻接块再轮到下一个核心块，这个两层布局是让 `conversation/budget.go` 的 `selectEvidence`（严格按输入顺序贪心消耗预算）天然保证"预算不足时核心块优先于所有邻接块、邻接块绝不挤出排名更低的核心块"的唯一原因 → 按 chunk ID 全局去重，再跑第二次 `dedupExactContentChunks`（同一个函数，对两层输出整体去重：核心块与邻接块正文重复时保留核心块，邻接块之间重复时保留输出顺序靠前者） | 返回核心命中 chunk 且融合排序正确（fusionScore 降序/Score 降序/ID 升序三级确定性排序）；`RetrievedChunk.Score` 始终是 0..1 相关度（向量命中=余弦相似度，关键词命中=word-similarity，两路命中取较大值；邻接块继承所属核心块的 Score，不冒充独立相关度），不泄漏 fusionScore；混合维度知识库互不串扰；embedding 服务失败时关键词一路仍可返回结果；两路都失败/无命中时返回空而非报错；正常路径无论多少个核心块/文档版本，邻接窗口只发生一次数据库批量查询；批量查询失败或核心块所属文档版本已被重新处理删除时静默降级为只返回核心块，绝不跨版本混入邻接内容；context 取消/超时在两个阶段都正常传播，不被吞掉；最终返回的 chunk 数量上界是 topK*3，不再是 topK；邻接块在预算不足时绝不能把排名更低的核心块挤出结果；正文完全相同（规范化后）的重复内容不重复出现在结果里，且去重从不改写保留条目的 Content/Score/Citation 元数据/DocumentVersion/NeighborOf |
