...
初版报告第 4 节把「孤儿行没有任何界面提示」列为本期最明显的可用性缺口并建议另立任务。
考虑到这个缺口是本期自己造出来的（§1.2 决定不级联删除），当期补掉更合适，故已补做。

**问题**：Agent 圈定的文档被删除后，范围记录**故意**保留（否则范围被删空会退回成
「不限定」，让 Agent 悄悄用起范围外的资料）。代价是用户看到的现象只是
「这个 Agent 突然什么都不知道了」，且完全无从排查。

**做法**：Agent 表单里，把已保存的范围 id 与该 Agent 所绑知识库下**当前存在**的
文档 id 求差集，差集非空时显示提示与一键清理按钮。

**一个必须区分的细节**：文档 id 查不到有两种原因，处理方式完全不同——

| 情况 | 处理 |
|---|---|
| 文档已被删除 | 提示「范围内有 N 份文档已不存在」+ 提供一键移除 |
| 文档还在，但状态是 pending/processing/failed | **不算失效**。列出来但禁用勾选，标明「处理中 / 处理失败，暂不可选」 |

早期实现里 `useDocumentsByKnowledgeBase` 直接在 hook 里就把非 `ready` 的文档滤掉了，
导致调用方无法区分这两者，会把一份**正在重新处理**的文档误判成已删除并建议用户移除——
那是一次真正的误删。现在 hook 返回全部状态，由调用方区分。

提示文案明确说明「留着它们是安全的（不会让 Agent 用到范围外的资料）」，
避免用户以为这是必须立刻处理的故障。

**验证**：在真实运行的服务上构造了完整场景——上传一份一次性文档、配进某 Agent 的范围、
删除该文档，然后在浏览器里确认：范围记录确实保留（API 回读仍是 2 个 id）、
表单显示红框提示「范围内有 1 份文档已不存在」与移除按钮、
同时 `hify_test.pdf` 因处理失败被列出但禁用。测试数据已清理。

**顺带修掉一个渲染缺陷**：提示文案里用了 Markdown 的 `**粗体**` 语法，
但 JSX 文本不解析 Markdown，页面上原样显示了星号。已改成 `<strong>`。
这类问题构建验证抓不到，只有真的把界面打开才会看见——与 Phase 11 §6.1 是同一类教训。
