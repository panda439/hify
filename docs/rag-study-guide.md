...
- **为什么轮次不够**：轮次封的是"跑了几趟"，不是"花了多少"。单趟可能就很贵——
  一个塞满检索结果和长工具返回值的调用，成本能顶十趟。反过来五趟快速失败的
  调用几乎不花钱却照样触顶。**轮次、时间、成本是三个正交维度**，任一失控都该停。
- **墙钟不用 `context.WithTimeout` 包 ctx，而是每轮开始检查一次**。因为 ctx 取消
  在这里已经是"客户端断开"的信号（`trySend` 靠它判断），把超时也塞进去会让两种
  完全不同的情况在日志里长得一样。单次调用慢是 provider 层的事（`resilience.go`
  有自己的超时与熔断），这一层管的是**累计**。
- **token 按「每次调用各自的 TotalTokens」累加**，不是按最终上下文长度算：
  工具循环每轮都会把完整历史重发，prompt token 是**反复计费**的，
  按调用累加才等于真实成本。
- **预算检查放在每轮开始而不是结束**，否则"已经超了"还会再白花一次模型调用
  （有专门用例锁定这一点）。
- ⚠️ **一条已知能力边界**：`provider.Usage` 是 best-effort，不是所有 OpenAI 兼容
  供应商都返回用量。拿不到用量时 token 预算**静默失效**——此时仍有轮次和墙钟
  两道防线兜底。这是取舍不是缺陷，但对外讲的时候要说出来。
- 三个上限目前是包内常量而不是配置项，与 `maxToolCallIterations` 保持一致。
  真要按部署环境调，再提升为配置。
