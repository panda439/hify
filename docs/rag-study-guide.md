...
| 3. 触顶降级 | 把已有中间结果交给模型，让它**基于现有信息带保留地作答**，并由程序强制标注信息不完整 | ❌ **是反的**——直接报错，连已生成的中间内容都不保存 |
...
**如果要补，建议顺序是 3 → 2 → 1**：第三层是纯体验收益、不增加任何模型调用；
第二层需要处理参数规范化（key 排序后序列化再哈希即可覆盖绝大多数真实死循环，
因为模型转圈时往往逐字相同，语义级去重属于过度设计）；第一层的 token/墙钟预算改动面最大。
