...
WHERE knowledge_base_id = ANY($2::text[])
  ...
  AND $1 <% content
  AND ($3::text[] IS NULL
       OR document_id = ANY($3::text[]))
  AND ($4::int IS NULL OR page_end     >= $4::int)
  AND ($5::int IS NULL OR page_number  <= $5::int)
...
type SearchKeywordChunksParams struct {
	...
	FilterPageMin     sql.NullInt32 `json:"filter_page_min"`
	FilterPageMax     sql.NullInt32 `json:"filter_page_max"`
...
func (q *Queries) SearchKeywordChunks(ctx context.Context, arg SearchKeywordChunksParams) ([]SearchKeywordChunksRow, error) {
	rows, err := q.db.QueryContext(ctx, searchKeywordChunks,
		arg.QueryText,
		pq.Array(arg.KnowledgeBaseIds),
		pq.Array(arg.FilterDocumentIds),
		arg.FilterPageMin,
		arg.FilterPageMax,
		arg.CandidateK,
	)
...
WHERE knowledge_base_id = ANY($2::text[])
  ...
  AND is_published = true
  AND ($4::text[] IS NULL
       OR document_id = ANY($4::text[]))
  AND ($5::int IS NULL OR page_end     >= $5::int)
  AND ($6::int IS NULL OR page_number  <= $6::int)
