...
// notice.go encodes and decodes the page lists behind Document's
// UnextractedPages and UnparseablePages — the two ways a page can fail to
// reach the knowledge base (007, then 008).
//
// ⭐ One pair of functions serves both columns, and the names say nothing
// about which. The two columns exist because their MEANINGS differ (one
// sends the user to OCR, the other tells them OCR will not help) — but
// "a sorted list of page numbers on disk" is one problem, and solving it
// twice would give the two copies a chance to drift apart.
//
...
// pageListSep is the on-disk separator. The stored form is
...
// encodePageList turns a page list into its stored form.
//
...
func encodePageList(pages []int) sql.NullString {
	if len(pages) == 0 {
	...
	return sql.NullString{String: strings.Join(parts, pageListSep), Valid: true}
}

// decodePageList reads the stored form back.
//
...
func decodePageList(raw sql.NullString) []int {
	if !raw.Valid || strings.TrimSpace(raw.String) == "" {
		return nil
	}
	fields := strings.Split(raw.String, pageListSep)
