...
1. **提示文案的确切措辞**。要同时满足 FR-008（能判断下一步动作）与 FR-009（说明有多少）。
   候选：「有 5 页未能提取文本（第 46-50 页），这些页可能是扫描图；如需检索其中内容，
   请用 OCR 工具转换后重新上传」。**过长会撑爆列表行**，过短又回到"只是焦虑"。
   建议：列表里给短版 + 完整页码走 `title` 悬浮，具体在 tasks 阶段定。
2. **连续页码是否折叠成区间**（`46,47,48,49,50` → `第 46-50 页`）。
   对扫描件这种"连着一段"的典型形态收益明显，但它是纯展示逻辑，
   需要确认放前端还是放 DTO——放前端则后端契约更干净，放 DTO 则两端不会渲染得不一致。
